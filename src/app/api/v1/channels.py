...
async def ingest_channel_payload(
    ...
    try:
        ...
        logger.warning(
            f"Channel normalization error for {channel_name}: {str(exc)}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
    except Exception as exc:
        logger.error(
            f"Unexpected error normalizing {channel_name}: {str(exc)}")
        raise HTTPException(
    ...
    try:
        ...
        logger.warning(
            f"Ingestion validation error for {channel_name}: {str(exc)}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
    except Exception as exc:
        logger.error(
            f"Unexpected error during ingestion for {channel_name}: {str(exc)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error"
        ) from exc
