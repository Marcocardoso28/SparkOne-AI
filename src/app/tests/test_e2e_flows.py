...
from ..models.db.messages import MessageRecord
...
class TestE2EFlows:
    ...
    async def test_e2e_whatsapp_to_task_creation(
        ...
        with patch("src.app.integrations.evolution_api.EvolutionAPIClient") as mock_evolution, \
            ...
            stmt = select(MessageRecord).where(
                MessageRecord.external_id == "test_message_id_001"
            )
    ...
    async def test_e2e_web_to_event_creation(
        ...
        """
        with patch("src.app.agents.orchestrator.Orchestrator") as mock_orchestrator, \
            ...
            # Validação de persistência da mensagem
            stmt = select(MessageRecord).where(
                MessageRecord.sender == "user@example.com"
            )
            result = await db_session.execute(stmt)
            message_record = result.scalar_one_or_none()
    ...
    async def test_e2e_sheets_sync_to_task_update(
        ...
        """
        with patch("src.app.channels.GoogleSheetsAdapter") as mock_sheets_adapter, \
             patch("src.app.services.tasks.TaskService") as mock_task_service:
    ...
    async def test_e2e_error_handling_and_fallback(
        ...
        with patch("src.app.agents.orchestrator.Orchestrator") as mock_orchestrator, \
            ...
            mock_orchestrator_instance = AsyncMock()
            mock_orchestrator.return_value = mock_orchestrator_instance
            mock_orchestrator_instance.handle.side_effect = Exception("Serviço temporariamente indisponível")
            ...
            stmt = select(MessageRecord).where(
                MessageRecord.sender == "test@example.com"
            )
            result = await db_session.execute(stmt)
            message_record = result.scalar_one_or_none()
...
async def clean_database():
    ...
    async for session in get_db_session():
        ...
        await session.execute("DELETE FROM messages WHERE sender LIKE '%test%' OR sender LIKE '%example.com'")
