...
async def get_performance_report(
    last_n_queries: Optional[int] = Query(None, ge=1, le=10000, description="Número de queries recentes a analisar"),
    current_user: dict = Depends(get_current_user)
) -> Dict[str, Any]:
    """
...
async def get_slow_queries(
    threshold: float = Query(0.5, ge=0.1, le=10.0, description="Threshold em segundos para considerar query lenta"),
...
async def get_query_details(
    ...
    try:
        if query_index < 0 or query_index >= len(db_profiler.query_profiles):
            raise HTTPException(
                status_code=404,
                detail=f"Query index {query_index} not found"
            )
        ...
        logger.info(f"Query details retrieved for index {query_index} by user: {current_user.get('id', 'unknown')}")
...
async def reset_profiler_stats(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Profiler statistics have been reset"
        }
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to reset profiler statistics"
        )
...
async def toggle_profiler(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Profiler has been {action}"
        }
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to toggle profiler"
        )
...
async def profiler_health() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "error",
            "message": "Failed to check profiler health",
            "error": str(e)
        }