...
MEMORY_USAGE = Gauge("sparkone_memory_usage_bytes", "Uso de memória da aplicação", ["component"])

CPU_USAGE = Gauge("sparkone_cpu_usage_percent", "Uso de CPU da aplicação", ["component"])
...
class DatabaseProfiler:
    ...
    def _setup_sqlalchemy_events(self):
        ...
        def before_cursor_execute(conn, cursor, statement, parameters, context, executemany):
            ...
            context._cpu_before = psutil.cpu_percent() if psutil is not None else None
        ...
        def after_cursor_execute(conn, cursor, statement, parameters, context, executemany):
            ...
            cpu_after = psutil.cpu_percent() if psutil is not None else None
    ...
    def _analyze_query(self, statement: str) -> dict[str, str]:
        ...
        return {"table": table, "operation": operation, "query_type": query_type}
    ...
    def _record_query_profile(self, profile: QueryProfile):
        ...
        QUERY_DURATION.labels(
            query_type=profile.query_type, table=profile.table, operation=profile.operation
        ...
        if profile.is_slow:
            SLOW_QUERIES.labels(query_type=profile.query_type, threshold="500ms").inc()
    ...
    def get_performance_report(self, last_n_queries: int | None = None) -> PerformanceReport:
        """
        ...
        cpu_samples = [q.cpu_percent for q in queries if q.cpu_percent is not None]
    ...
    async def async_wrapper(*args, **kwargs):
        ...
        try:
            ...
            if duration > db_profiler.slow_query_threshold:
                profiler_logger.warning(f"Slow function: {func.__name__} took {duration:.3f}s")

            return result

        except Exception as e:
            duration = time.time() - start_time
            profiler_logger.error(f"Function {func.__name__} failed after {duration:.3f}s: {e}")
            raise
...
async def profile_session(
    session: AsyncSession, operation_name: str
    ...
    try:
        ...
        REQUEST_LATENCY.labels(method=operation_name, endpoint="database_session").observe(duration)

    except Exception as e:
        duration = time.time() - start_time
        profiler_logger.error(f"Session {operation_name} failed after {duration:.3f}s: {e}")
        raise
...
async def analyze_slow_queries(threshold_seconds: float = 0.5) -> list[QueryProfile]:
    ...
    """
    slow_queries = [
        profile for profile in db_profiler.query_profiles if profile.duration > threshold_seconds
    ]

    # Ordenar por duração (mais lentas primeiro)
    slow_queries.sort(key=lambda q: q.duration, reverse=True)

    profiler_logger.info(f"Found {len(slow_queries)} slow queries (>{threshold_seconds}s)")
